...
function ExportManager:DisplayExportManager()
    if (Functions:CountTable(ItemManager.CurrentItems) == 0) then return end

    local ProfileName = Functions:Condition(ItemManager.IsNewProfile, "New Profile", ItemManager.CurrentProfileName)
    local Profile = {}
    Profile[ProfileName] = {}
    Profile[ProfileName].Items = ItemManager.CurrentItems

    local ExportedProfile = ExportManager:ExportProfile(ProfileName, Profile)
...
function ExportManager:ExportProfile(ProfileName, Profile)
    local JsonProfile = Json:Encode(Profile[ProfileName])
...
function ExportManager:ImportProfile(EncodedProfile)
    ...
    if (strlen(ProfileName) == 0 or LootAlarmLocalDB.Profiles[ProfileName]) then
        ProfileName = "New profile"
        local i = 1
        while (LootAlarmLocalDB.Profiles[ProfileName]) do
            i = i + 1
            ProfileName = "New profile "..i
        end
    end

    LootAlarmLocalDB.Profiles[ProfileName] = Json:Decode(JsonProfile)
